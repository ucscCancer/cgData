...
def writeProbeMap( h5, probeMap ):
	...
	probeType = numpy.dtype( [ ('hugo', 'S') ] ) #,  ('chrome', 'S'), ('start', 'i'), ('stop', 'i'), ('strand', 'S') ] )
	...
	ds = h5.create_dataset( "/probeMap/%s" % ( probeMap.attrs['name'] ), [pmCount], dtype=hProbeType )
	
	i = 0
	val = numpy.zeros(1, dtype=probeType)
	for probe in probes:
		print i
		ds[ i ] = i #, 'hugo' ] = i # probeMap.geneMap[ probe ]
		#ds.attrs[ probe ] = i
		i += 1
	

def writeGeneMatrix( h5, gm ):
	ds = h5.create_dataset( "%s" % ( gm.attrs['name'] ), [ len( gm.probeHash ), len( gm.sampleList ) ], dtype=float )	
	i = 0
	for probe in gm.probeHash:
		rowHash = gm.probeHash[ probe ]
		row = numpy.zeros( len( gm.sampleList ) )		
		for j in range( len( gm.sampleList ) ):
			if rowHash.has_key( gm.sampleList[j] ): 
